...
### 2. Ограничения paramètreов (where)

**Метод:** `where(string|array $parameter, ?string $pattern = null): Route`

**Описание:** Добавление регулярных выражений для валидации paramètreов.
...
**Important:**
- Опциональные paramètres должны быть в конце URI
...
**Описание:** Installation значений по умолчанию для paramètreов.
...
**Использование:**
- Упрощение обработки опциональных paramètreов
- Fallback значения
- Конфигурация по умолчанию

---

### 6. Получение paramètreов

**Методы:**
- `Route::getParameters(): array`
- `Route::getParameter(string $name, mixed $default = null): mixed`

**Описание:** Получение значений paramètreов из объекта Route.
...
1. **Всегда валидируйте paramètres**
   ```php
   // ✅ Хорошо
...
1. **Не делайте paramètres слишком общими**
   ```php
   // ❌ Плохо - ловит всё
   Route::get('/files/{path}', $action);
...
2. **Не используйте опциональные paramètres в середине**
   ```php
   // ❌ Плохо - не работает
...
|----------|-------|-----------|
| Парсинг paramètreов | ~1-2μs | Очень быстро |
