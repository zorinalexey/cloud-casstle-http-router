...
// GET /users -> автоматически именуется как 'users.get'
$router->get('/users', 'UserController@index');

// GET /api/v1/users/{id} -> автоматически именуется как 'api.v1.users.id.get'
$router->get('/api/v1/users/{id}', 'ApiController@show');

// Использование автогенерированных имён
$route = $router->getRouteByName('api.v1.users.id.get');
```

**Преимущества:**
- ✅ Экономия времени - не нужно именовать маршруты вручную
- ✅ Консистентность - единый формат имён
- ✅ Предсказуемость - понятные и логичные имена
- ✅ Гибкость - можно переопределить любое имя
